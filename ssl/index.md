...
いわゆる「IP一つで複数のSSL証明書を利用する」方法。
現在はIP一つ（というより、ポート一つ）でSSLは１つしか設定できないが、SNIを利用することで、IP一つで複数のSSL証明書を利用する事ができる。

ただし、これはサーバ側、クライアント側両方の対応が必要になる

20140709現在、市場にはXPとAndroid2系がまだ残存していることから、積極的な採用は難しいと思われる。
...
-------|---
Apache|2.2.12以上
nginx|0.5.32以上かつビルドオプションを有効

> また、OpenSSL>0.9.8f以下はこちらもビルドオプションが必要な模様
